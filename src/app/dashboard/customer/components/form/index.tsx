...
const schema = z.object({
    ...
    phone: z.string().refine((value) => {
        return /^(?:\(\d{2}\)\s?)?\d{9}$/.test(value) || /^\d{s}\s\d{9}$/.test(value) || /^\d{11}$/.test(value)
    }, {
        message: 'O numero de telefone deve ter (DD)999999999'
    }),
    address: z.string()
})
            ...
            <section className='flex gap-2 my-2 flex-col sm:flex-row'>
                <div className='flex-1'>
                    ...
                    <Input
                        type='text'
                        name='phone'
                        placeholder='Telefone Ex.: (DD)999999999'
